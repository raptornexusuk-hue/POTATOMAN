...
// Origins allowed to reach /api from another host, comma-separated, e.g. the domain the game
// itself is uploaded to when this service runs somewhere else.
const ORIGINS=(process.env.POTATOMAN_ALLOWED_ORIGINS||'').split(',').map(v=>v.trim().replace(/\/+$/,'')).filter(v=>/^https?:\/\/[^/]+$/.test(v));
...
createServer(async(req,res)=>{try{const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>110000){res.writeHead(413);res.end('Request too large');return;}chunks.push(chunk);}const origin=process.env.POTATOMAN_PUBLIC_ORIGIN||`http://${req.headers.host}`,url=new URL(req.url,origin);const request=new Request(url,{method:req.method,headers:req.headers,...(!['GET','HEAD'].includes(req.method)?{body:Buffer.concat(chunks)}:{})});const response=await worker.fetch(request,{DB,ASSETS,ORIGINS,ORIGIN:process.env.POTATOMAN_PUBLIC_ORIGIN||origin,MAIL_URL:process.env.POTATOMAN_MAIL_URL||'',MAIL_TOKEN:process.env.POTATOMAN_MAIL_TOKEN||''});res.writeHead(response.status,Object.fromEntries(response.headers));res.end(Buffer.from(await response.arrayBuffer()));}catch{res.writeHead(500);res.end('Server error');}}).listen(Number(process.env.PORT)||3000,'0.0.0.0',()=>console.log('Potatoman server ready.'));
