...
a.setPause(false);a.setClock(119.999);a.tick(dt);assert.equal(a.snapshot().state,'results');a.next();assert.equal(a.snapshot().bonus,false,'a hunt does not follow every single round');
// The hunt runs once every four rounds, on an arena of its own rather than the ground just fought
// over, so a circuit is hunted across several worlds instead of the same one again and again.
{
 const arenas=new Set();let hunts=0;
 for(let level=0;level<LEVELS.length-1;level++){
  a.init(level);a.setClock(119.999);a.tick(dt);assert.equal(a.snapshot().state,'results');a.next();
  const hunt=a.snapshot().bonus;
  assert.equal(hunt,(level+1)%4===0,`round ${level+1} ${hunt?'ran':'skipped'} a hunt`);
  if(!hunt)continue;
  hunts++;arenas.add(a.snapshot().arena);
  assert.notEqual(a.snapshot().arena,LEVELS[level].name,`round ${level+1} hunted the very ground it just fought over`);
  assert.equal(a.snapshot().remaining,BONUS_TIME);
  a.finishBonus(false);a.next();assert.equal(a.snapshot().bonus,false);
 }
 assert.equal(hunts,5,`a twenty-one round circuit runs five hunts, not ${hunts}`);
 assert.ok(arenas.size>=4,`and spreads them across the worlds: ${arenas.size} different arenas`);
}
a.init(0);a.setClock(119.999);a.tick(dt);a.next();
console.log('PASS a hunt every fourth round, on a rotating arena, and normal throws after a hunter win');
