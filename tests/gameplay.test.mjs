...
for(const file of['progression','map-catalogue','camera-input','game-audio','difficulty','weapons','stance','aiming','weapon-pickup','spatial-audio'])code=code.replace(`'./${file}.js'`,JSON.stringify(new URL(`../dist/${file}.js`,import.meta.url).href));
...
a.setDuration(90);a.init(0);assert.equal(a.snapshot().remaining,90);a.setDuration(300);assert.equal(a.snapshot().remaining,90);a.setClock(89.999);a.tick(dt);a.next();assert.equal(a.snapshot().remaining,40);a.finishBonus(false);a.next();assert.equal(a.snapshot().remaining,300);console.log('PASS level duration is captured per round; bonus remains 40 seconds');
...
let me=a.localPlayer();me.panX=1;me.pitch=.4;a.keys.add(a.getSettings().keys[0].resetCamera);a.guestFrame(dt);a.keys.clear();assert.equal(me.panX,0);assert.equal(me.pitch,-.08);
...
for(const seed of[3,717,90001]){a.init(9,false,seed);let t=0;while(a.snapshot().state==='playing'&&t++<14402)a.tick(dt);assert.ok(a.snapshot().players.slice(1).every(p=>Number.isFinite(p.best)),`Remixed finale ${seed} must be completable`);}
