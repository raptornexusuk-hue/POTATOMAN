...
const p={x:3,z:4,yaw:.5,pitch:.1,panX:0,panY:0};cameraDrag(p,50,-20,'pan',defaults());assert.equal(p.x,3);assert.equal(p.z,4);assert.equal(p.yaw,.5);assert.equal(p.pitch,.1);assert.notEqual(p.panX,0);assert.notEqual(p.panY,0);cameraDrag(p,1e6,1e6,'pan',defaults());assert.equal(p.panX,-2);assert.equal(p.panY,1.2);const old=p.yaw;cameraDrag(p,20,10,'look',defaults());assert.notEqual(p.yaw,old);resetCamera(p);assert.equal(p.panX,0);assert.equal(p.panY,0);assert.equal(p.cameraDistance,4.6);console.log('PASS camera pan does not change movement or aim; bounds/reset work');
