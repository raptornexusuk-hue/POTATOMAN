...
assert.equal(MAPS.length,4);assert.equal(new Set(LEVELS.map(mapId)).size,4);
for(const previous of MAPS.map(m=>m.id))for(let trial=0;trial<100;trial++){
 ...
 const circuit=circuitLevels(seed);assert.equal(new Set(circuit).size,10);assert.equal(circuit.at(-1),9);
 assert.equal(new Set(circuit.map(i=>mapId(LEVELS[i]))).size,4);
}
...
console.log('PASS four world families, all ten rounds, 400 non-repeating openings and authored combat route/collider consistency');
