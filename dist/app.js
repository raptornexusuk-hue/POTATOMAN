...
$('levelsButton').onclick=()=>dialog('levelsDialog');$('controlsButton').onclick=openSettings;$('artButton').onclick=()=>dialog('artDialog');document.querySelectorAll('.close').forEach(b=>b.onclick=()=>{if(b.closest('dialog').id==='settingsDialog')closeSettings();else b.closest('dialog').close();});
function durationLabel(seconds){return `${seconds/60}-minute`;}
function refreshDuration(){setPlayers(duo);$('durationSummary').textContent=`${settings.roundSeconds/60} minutes`;document.querySelectorAll('[data-level-duration]').forEach(el=>el.textContent=`${settings.roundSeconds/60} MIN`);}
...
const WORLD_ICON={village:'🧀',estate:'🌷',harbour:'⚓',farm:'🌾'},MODE_ICON={battle:'⚔️',race:'🏁',assault:'🪜',capture:'👑',smash:'💥'};
function renderWorlds(id='all'){selectedWorld=id;$('mapTabs').innerHTML=[{id:'all',name:'ALL WORLDS'},...MAPS].map(m=>`<button data-world="${m.id}" class="${id===m.id?'active':''}" aria-pressed="${id===m.id}">${m.id==='all'?'🌍 ':WORLD_ICON[m.id]+' '}${m.name}</button>`).join('');$('mapDescription').textContent=MAPS.find(m=>m.id===id)?.description??'Choose the place first, then a game mode. Circuit play mixes all four worlds.';$('levelGrid').innerHTML=LEVELS.map((l,i)=>({l,i})).filter(({l})=>id==='all'||mapId(l)===id).map(({l,i})=>`<button class="level-card" data-level="${i}" style="--map-color:${mapInfo(l).color}"><span class="level-num">${String(i+1).padStart(2,'0')}</span><span><small>${mapInfo(l).name.toUpperCase()}</small><strong>${MODE_ICON[l.mode]??''} ${MODES[l.mode]}</strong><small>${l.name} · <span data-level-duration>${settings.roundSeconds/60} MIN</span></small><span class="level-detail">${l.skill}</span></span></button>`).join('');}
...
function loadRound(isBonus,hostDuration=null){
 ...
 players.forEach(p=>{Object.assign(p,{score:0,kills:0,best:Infinity,throwCD:0,catchCD:0,catchTime:0,dashCD:0,dashTime:0,weapon:'throw',weaponLevel:0,gun:false,mag:12,reload:0,runner:bonus&&p.id===runnerId,checks:[false,false],shotAnim:0,pendingThrow:0,botDelay:1.7+Math.max(0,7-levelIndex)*.4});});
...
function botInput(p,dt){const ai=aiSettings(settings.difficulty,levelIndex);p.aiThink-=dt;p.aiFire=Math.max(0,p.aiFire-dt);const mode=currentLevel().mode;const enemies=players.filter(q=>q.id!==p.id&&q.respawn<=0&&(!bonus||(p.runner?true:q.runner)));let goal,enemy=enemies.sort((a,b)=>Math.hypot(a.x-p.x,a.z-p.z)-Math.hypot(b.x-p.x,b.z-p.z))[0];
 ...
 if(p.aiThink<=0){p.aiThink=ai.reaction;p.aiAim=canShoot?{x:aim.x+Math.sin(time*1.3+p.id*4)*ai.error,z:aim.z+Math.cos(time*.9+p.id*3)*ai.error}:null;}
...
function tick(dt){readInputs(dt);if(paused&&(!online||online.isHost))return;if(online&&paused)Object.assign(inputs[0],{x:0,z:0,fire:false,catch:false,dodge:false,jump:false,crouch:false});if(introRemaining>0){introRemaining=Math.max(0,introRemaining-dt);return;}time+=dt;remaining=Math.max(0,remaining-dt);const mode=currentLevel().mode;
 ...
 if(!bonus&&isTrial(currentLevel())&&(mode!=='assault'||p.courseStep===map.course.length)&&Math.hypot(p.x-map.exit.x,p.z-map.exit.z)<1.25){p.best=Math.min(p.best,p.attempt);p.score++;p.points+=300+Math.max(0,Math.round(120-p.attempt));p.respawn=1;world.burst(p.x,p.z,0x7dedc8,16);sound('win',p);if(p.id===(online?.slot??0))quip(`${p.name}: ${p.best.toFixed(2)} seconds. Go again!`,true,'Running down the fine river of butter');}
 if(bonus&&p.runner){world.checkpoints.forEach((c,i)=>{if(!p.checks[i]&&Math.hypot(p.x-c.position.x,p.z-c.position.z)<1.5){p.checks[i]=true;c.visible=false;sound('pickup',p);}});if(p.checks.every(Boolean)&&Math.hypot(p.x-map.exit.x,p.z-map.exit.z)<1.5){finishBonus(true);return;}}
 }
...
function updateHUD(){if(!players.length)return;const l=currentLevel(),mode=l.mode;hudText('roundIndex',bonus?'BONUS ROUND':`ROUND ${String(levelIndex+1).padStart(2,'0')} / ${LEVELS.length}`);hudText('levelName',bonus?'The Great Spud Escape':l.name);hudText('modeName',bonus?'HUNT POTATOMAN':MODES[mode]);hudText('timer',`${Math.floor(Math.ceil(remaining)/60)}:${String(Math.ceil(remaining)%60).padStart(2,'0')}`);$('timer').style.color=remaining<30?'#ff8c6e':'';$('timerWrap').classList.toggle('critical',remaining<=10);
 ...
 hudText('objective',bonus?'Runner: 2 checkpoints → exit. Hunters: stop him.':({assault:`Land on ${map.course.length} numbered platforms in order · fastest run wins`,race:`Fastest escape in ${activeDuration/60} minutes wins · repeat to improve`,battle:'Most knockouts wins · one shared weapon box at the centre',capture:'Hold the gold zone · contested earns nothing',smash:'Break the gold-ringed crates'})[mode]);
...
async function drainSnapshots(){applyingSnapshot=true;try{while(pendingSnapshot&&!arenaFailed&&online&&!online.isHost){const {snapshot:s,connection}=pendingSnapshot;pendingSnapshot=null;if(connection!==online)continue;
 ...
 if(bonus)world.checkpoints.forEach((c,i)=>c.visible=!players[levelIndex%players.length].checks[i]);if(!bonus&&currentLevel().mode==='capture'){const pos=zonePosition();world.zone.position.set(pos.x,.03,pos.z);}
...
function updatePlayabilityHUD(){if(!map||!players.length)return;$('roundIntro').hidden=introRemaining<=0;
 $('damageFlash').style.opacity=Math.max(0,damageFlashUntil-performance.now())/260*.5;hudText('introTitle',bonus?'The Great Spud Escape':currentLevel().name);hudText('introMode',bonus?'BONUS HUNT':MODES[currentLevel().mode]);hudText('introDetail',bonus?'Runner: collect both checkpoints, then escape. Hunters: race for the one shared gun; recover it when its holder falls.':currentLevel().detail);hudText('introCount',Math.ceil(introRemaining));$('hitFeedback').hidden=performance.now()>hitUntil;
 const p=localPlayer();if(!bonus&&isTrial(currentLevel()))world.markTrail?.(p);let goal,label='';if(bonus){if(p.runner){const index=p.checks.findIndex(c=>!c);goal=index<0?map.exit:world.checkpoints[index].position;label=index<0?'EXIT':'CHECKPOINT';}else{goal=players[runnerId];label='POTATOMAN';}}else if(isTrial(currentLevel())){goal=currentLevel().mode==='assault'?(map.course[p.courseStep]??map.exit):map.exit;label=currentLevel().mode==='assault'&&p.courseStep<map.course.length?(map.course[p.courseStep].duck?'DUCK GATE ':'PLATFORM ')+(p.courseStep+1):'EXIT';}else if(currentLevel().mode==='capture'){goal=zonePosition();const inside=players.filter(p=>p.respawn<=0&&Math.hypot(p.x-goal.x,p.z-goal.z)<3);label=inside.length>1?'CONTESTED':'CROP ZONE';if(levelIndex>=6)label+=` · MOVES IN ${45-Math.floor(time%45)}s`;}
