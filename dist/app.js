...
let circuitSeed=0;
// The hunt is no longer tied to the round that came before it. It moves round the circuit on its
// own stride, so the arena for a hunt is rarely the one just fought over, and a full circuit hunts
// across several different worlds rather than the same ones every time.
const HUNT_EVERY=4,HUNT_STRIDE=5;
let huntIndex=0;
const huntFollows=index=>index<LEVELS.length-1&&(index+1)%HUNT_EVERY===0;
...
// The kit the person on this device wears. Their rivals wear the four stock ones from the same rack.
let outfit=loadOutfit(preferenceStorage),lockerSeen=!!preferenceStorage?.getItem('potatoman.outfit.v1');
...
for(const id of['quality','sensitivity','panSensitivity','sound','voice','music','musicVolume','voiceVolume','invertY','trail','difficulty','fov','zoom'])$(id).addEventListener(['sensitivity','panSensitivity','musicVolume','voiceVolume','fov','zoom'].includes(id)?'input':'change',e=>{settings[id]=['sound','voice','music','invertY','trail'].includes(id)?e.target.checked:['quality','difficulty'].includes(id)?e.target.value:+e.target.value;cameraValues();if(id==='quality')world?.quality(settings.quality);if(id==='difficulty'){refreshDifficulty();setPlayers(duo);}if(id==='voice'&&!settings.voice||id==='voiceVolume'&&settings.voiceVolume<=0)gameAudio.stopVoice();initAudio();syncAudio();persist();});
...
async function start(index,practice=false){huntIndex=(circuitSeed+2)%LEVELS.length;
...
function loadRound(isBonus,hostDuration=null){
 ...
 audioEvents=[];audioEventId=0;guestAudioWatermark=0;markedUntil=0;gameAudio.stopEffects();bonus=isBonus;paused=false;state='playing';time=0;lastQuip=-99;killFeed=[];damageFlashUntil=0;acc=0;last=performance.now();activeDuration=bonus?BONUS_TIME:(hostDuration??settings.roundSeconds);remaining=activeDuration;lastUI=0;clearInput();$('caption').textContent='';runnerId=levelIndex%players.length;map=playableMap(arenaLevel(),bonus,activeDuration);shots=[];targets=[];pickups=[];
 players.forEach(p=>{Object.assign(p,{score:0,kills:0,best:Infinity,throwCD:0,catchCD:0,catchTime:0,dashCD:0,dashTime:0,weapon:'throw',weaponLevel:0,gun:false,mag:12,reload:0,runner:bonus&&p.id===runnerId,checks:[false,false],claimTime:0,shotAnim:0,pendingThrow:0,botDelay:1.7+Math.max(0,1-circuitProgress(levelIndex)*9/7)*2.8});});
 world.build(map,arenaLevel(),players,bonus);players.forEach(p=>spawn(p,!bonus&&isTrial(currentLevel())));
...
document.addEventListener('mouseup',e=>{mouseButtons=e.buttons??0;});document.addEventListener('mousemove',e=>{if(e.movementX||e.movementY||e.sourceCapabilities&&!e.sourceCapabilities.firesTouchEvents)lookDevice[0]='mouse';if(mouseReady()&&Number.isInteger(e.buttons))mouseButtons=e.buttons;});
...
// Which device last aimed each view. A controller merely plugged in is not a controller being used,
// and auto-levelling a mouse player's pitch while they walk is exactly the fight it must not pick.
let lookDevice=['keyboard','keyboard'];
...
function readInputs(dt){const pads=gamepads(),assignment=assignedPads(pads);if(paused||document.querySelector('dialog[open]')){inputs.forEach(o=>Object.assign(o,{x:0,z:0,fire:false,catch:false,dodge:false,jump:false,crouch:false}));const held=pads.some(p=>p.buttons[9]?.pressed);if(paused&&held&&!padPause&&!$('settingsDialog').open&&!$('controllerDialog').open)resume();padPause=held;return;}
 assisted=[isTouch||lookDevice[0]==='pad',isTouch||lookDevice[1]==='pad'];steering=[isTouch&&aimPointer!==null,false];
...
function actions(p,input,index){if(updateStance(p,input.crouch,[...map.walls,...map.platforms]))sound('crouch',p);if(input.resetCamera&&!prior[index]?.resetCamera)resetView(p);if(input.jump&&!prior[index]?.jump&&jump(p))sound('jump',p);if(p.respawn>0)return;const race=!bonus&&isTrial(currentLevel());if(input.dodge&&!prior[index]?.dodge&&p.dashCD<=0){let dx=input.x,dz=input.z,l=Math.hypot(dx,dz);if(l<.1){dx=Math.sin(p.yaw);dz=-Math.cos(p.yaw);l=1;}p.dashX=dx/l;p.dashZ=dz/l;p.dashTime=.16;p.dashCD=p.runner?2.6:2;sound('dash',p);}
 if(!race){if(input.fire&&!p.runner)fire(p);if(input.catch&&!prior[index]?.catch&&p.catchCD<=0&&!p.runner){p.catchTime=circuitProgress(levelIndex)<1/3?.26:.21;p.catchCD=.8;}}
...
// The zone only starts moving in the last third of the circuit. The HUD has to ask the same
// question, or it counts players down to a move that never comes.
const zoneMoves=()=>circuitProgress(levelIndex)>=2/3;
function zonePosition(){if(!zoneMoves())return{x:0,z:0};const n=Math.floor(time/45)%3;return n===0?{x:0,z:0}:map.toWorld(n===1?3:map.n-4,Math.floor(map.n/2));}
...
function tick(dt){readInputs(dt);if(paused&&(!online||online.isHost))return;if(online&&paused)Object.assign(inputs[0],{x:0,z:0,fire:false,catch:false,dodge:false,jump:false,crouch:false});if(introRemaining>0){introRemaining=Math.max(0,introRemaining-dt);return;}time+=dt;remaining=Math.max(0,remaining-dt);const mode=currentLevel().mode;moveCourse(map,time);
 ...
 let input=isHuman(p)?(online&&p.id!==online.slot?remoteInput(p):inputs[online?0:p.id]):botInput(p,dt);actions(p,input,p.id);let speed=p.runner?8.5:6;if(!isHuman(p)){const ai=aiSettings(settings.difficulty,circuitProgress(levelIndex));speed*=p.runner?ai.runner:ai.speed;}
...
function finishMain(){if(state!=='playing'||online&&!online.isHost)return;releaseMouse();state='results';clearInput();const mode=currentLevel().mode,sorted=[...players].sort((a,b)=>isTrial(currentLevel())?a.best-b.best:b.score-a.score);const best=isTrial(currentLevel())?sorted[0].best:sorted[0].score;const winners=sorted.filter(p=>isTrial(currentLevel())?Number.isFinite(p.best)&&Math.abs(p.best-best)<STEP:Math.abs(p.score-best)<STEP);for(const p of winners){p.total+=3;p.points+=300;}awardRoundWins(players,winners);const closingLine=lastPlaceLine(players,isTrial(currentLevel()),levelIndex);if(closingLine)gameAudio.speak(closingLine,{force:true,role:'commentary'});
 ...
 else if(huntFollows(levelIndex)){$('nextRound').textContent='BONUS HUNT →';
  huntIndex=(huntIndex+HUNT_STRIDE)%LEVELS.length;if(huntIndex===levelIndex)huntIndex=(huntIndex+1)%LEVELS.length;
  resultAction=()=>loadRound(true);$('resultText').textContent+=` Next: a hunt across ${roundLevel(huntIndex,circuitSeed).name}. ${players[levelIndex%players.length].name} becomes Potatoman.`;}
 else{$('nextRound').textContent='NEXT LEVEL →';resultAction=()=>{levelIndex++;loadRound(false);};}if(closingLine)$('resultText').textContent+=' '+closingLine;sound('win');captureResult();resultData.commentary=closingLine;recordPlayerRound();playerAccount.checkpoint();dialog('resultDialog');}
