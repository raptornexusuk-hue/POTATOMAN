...
function updateHUD(){if(!players.length)return;const l=currentLevel(),mode=l.mode;hudText('roundIndex',bonus?'BONUS ROUND':`ROUND ${String(levelIndex+1).padStart(2,'0')} / ${LEVELS.length}`);hudText('levelName',bonus?'The Great Spud Escape':l.name);hudText('modeName',bonus?'HUNT POTATOMAN':MODES[mode]);hudText('timer',`${Math.floor(Math.ceil(remaining)/60)}:${String(Math.ceil(remaining)%60).padStart(2,'0')}`);$('timer').style.color=remaining<30?'#ff8c6e':'';$('timerWrap').classList.toggle('critical',remaining<=10);
 ...
 hudText('objective',bonus?'Runner: 2 checkpoints → exit. Hunters: stop him.':({climb:`Climb all ${map.course.length} platforms to the top of the tower · fastest ascent wins`,assault:`Land on ${map.course.length} numbered platforms in order · fastest run wins`,race:`Fastest escape in ${activeDuration/60} minutes wins · repeat to improve`,battle:'Most knockouts wins · one shared weapon box at the centre',capture:'Hold the gold zone · contested earns nothing',smash:'Break the gold-ringed crates'})[mode]);
