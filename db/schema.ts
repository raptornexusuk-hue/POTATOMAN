...
export const profiles=sqliteTable('profiles',{
 ...
 email:text('email'),emailKey:text('email_key'),verified:integer('verified').notNull().default(0),verifyToken:text('verify_token'),verifySent:integer('verify_sent').notNull().default(0)
},t=>[index('idx_profiles_token').on(t.token),uniqueIndex('idx_profiles_email').on(t.emailKey),index('idx_profiles_verify').on(t.verifyToken)]);
